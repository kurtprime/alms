...
export default function ClassSubjectClient({ session }: { session: Session }) {
  ...
  const { data } = useSuspenseQuery(trpc.user.getCurrentSectionInfo.queryOptions());

  return (
    <>
      {data.map((subject) => {
        return <SidebarMenuItemInfo subject={subject} key={subject.id} session={session} />;
      })}
    </>
  );
}
      ...
      >
        <Link href={`/class/${subject.id}`} className="w-full">
          ...
          <span className="text-muted-foreground text-xs">
            ({isTeacher ? subject.enrolledClassName : subject.teacherName})
          </span>
        </Link>
      </SidebarMenuButton>
      <SidebarMenuBadge className="pr-3">{subject.subjectCode}</SidebarMenuBadge>
    </SidebarMenuItem>
  );
}
