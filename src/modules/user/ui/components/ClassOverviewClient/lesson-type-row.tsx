...
export function LessonTypeRow({
  ...
  const now = useNow();
  const endDateString =
    item.type === 'quiz' ? item.quizSettings?.endDate : item.assignmentSettings?.endDate;
  ...
  const { isPastDue, isDueSoon, formattedDate } = useMemo(() => {
    if (!dueDate) return { isPastDue: false, isDueSoon: false, formattedDate: null };
      ...
      >
        ...
        <div
          className={cn(
            ...
            isCompleted ? 'bg-green-500' : 'bg-slate-200 group-hover:bg-slate-300'
          )}
        />
